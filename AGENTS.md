...
Client-management platform for Anthony Multiservice, LLC — a notary/apostille, immigration, tax prep, and credit/financing business in Kissimmee, FL. See `README.md` for the full stack, module list, and setup steps.
...
- **Design tokens:** mint/white/coral/dark-green brand tokens live in `src/app/globals.css`. Single light theme, no dark mode. `--accent` is a lightened coral tint (not the literal brand coral, which is `--chart-2`) specifically so every existing `bg-accent`/`text-accent-foreground` badge/chip passes AA — see the comment at the top of the token block for the full contrast rationale before changing any token.
